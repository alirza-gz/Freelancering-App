  ...
  return (
    open && (
      ...
      <div className="w-full h-screen bg-primary-300 bg-opacity-30 z-40 backdrop-blur-sm fixed top-0 left-0">
        ...
        <div
          ref={ref}
          className="w-[calc(100vw-2rem)] md:max-w-lg max-h-[calc(100vh-2rem)] overflow-y-auto fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-secondary-0 rounded-lg shadow-lg transition-all duration-500 ease-in-out"
        >
          {/* Header */}
          <div className="flex-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
            <h3 className="flex-center flex-1 text-xl font-MorabbaBold text-secondary-700 dark:text-white">
              {title}
            </h3>
            <button
          ...
          {/* Body */}
          <div className="p-4 md:p-5 flex flex-col gap-y-4 text-secondary-700">
            {children}
          </div>
        </div>
      </div>
